...
class WidgetEntry:
    def __init__(self, text="", target=None, clickfunc=None):
        ...
        if self.clickfunc:
            self.irrel_button.on_click(partial(self.clickfunc, self.target, False))

        self.rel_button = widgets.Button(description="Relevant")
        if self.clickfunc:
            self.rel_button.on_click(partial(self.clickfunc, self.target, True))
